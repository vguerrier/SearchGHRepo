...
Public Class FsearchSelectPatch
    ...
    Function RequestPatch(Patch As String) As String
        ...
        RequestPatch = RequestPatch & " union select a.numeroreference, a.numerofiche, c.nom , a.titre, r.nom from cqgtop.anomalie a, cqgtop.client c, cqcentral.rfe r "
        RequestPatch = RequestPatch & " where a.rfecode = r.dbid and a.client_champ = c.dbid and a.identificationexterne = '" + Patch + "'"
        RequestPatch = RequestPatch & " union select a.numeroreference, a.numerofiche, c.nom , a.titre, r.nom from CQGOLDSTOCK.anomalie a, CQGOLDSTOCK.client c, cqcentral.rfe r "
        RequestPatch = RequestPatch & " where a.rfecode = r.dbid and a.client_champ = c.dbid and a.identificationexterne = '" + Patch + "'"
        RequestPatch = RequestPatch & " union select a.numeroreference, a.numerofiche, c.nom , a.titre, r.nom from cqgtrack.anomalie a, cqgtrack.client c, cqcentral.rfe r "
        RequestPatch = RequestPatch & " where a.rfecode = r.dbid and a.client_champ = c.dbid and a.identificationexterne = '" + Patch + "'"
        RequestPatch = RequestPatch & " union select a.numeroreference, a.numerofiche, c.nom , a.titre, r.nom from cqcas.anomalie a, cqcas.client c, cqcentral.rfe r "
        RequestPatch = RequestPatch & " where a.rfecode = r.dbid and a.client_champ = c.dbid and a.identificationexterne = '" + Patch + "'"
        RequestPatch = RequestPatch & " union select a.numeroreference, a.numerofiche, c.nom , a.titre, r.nom from CQGOLDEVENTS.anomalie a, CQGOLDEVENTS.client c, cqcentral.rfe r "
        RequestPatch = RequestPatch & " where a.rfecode = r.dbid and a.client_champ = c.dbid and a.identificationexterne = '" + Patch + "')"
